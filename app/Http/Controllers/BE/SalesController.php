...
class SalesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $users = User::whereIn('role', [UserConstant::TELESALES, UserConstant::WAITER])->withTrashed()->get()->map(function ($item) use ($request) {
            ...
            $item->schedules_new = Schedule::select('id')->where('creator_id', $item->id)->whereIn('user_id', $data_new->pluck('id')->toArray())->whereBetween('date', getTime($request->data_time))->get()->count();//lich hen
