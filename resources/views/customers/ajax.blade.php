            ...
            @foreach($customers as $key => $customer)
                <tr class="{{$customer->expired_time_boolean == 1 ? 'qua-han' : ''}}">
                    ...
                    <td class="text-center phone-customer {{in_array(3,$user_filter_grid) ? '':'display-none'}}" data-phone="{{$customer->phone}}"
                        ...
                        <a class="hide-href" href="javascript:void(0)">
                            {{ auth()->user()->permission('phone.open')? $customer->phone :str_limit($customer->phone,7,'xxx') }}
                        </a>
...
{{--                    <td class="text-center {{in_array(10,$user_filter_grid) ? '':'display-none'}}">{{@$customer->time_move}}</td>--}}
                    ...
                    <td class="text-center genitive-db {{in_array(13,$user_filter_grid) ? '':'display-none'}}" data-id="{{@$customer->id}}">
                        {{count($customer->genitive)?implode(', ',$customer->genitive->pluck('name')->toArray()):''}}</td>
