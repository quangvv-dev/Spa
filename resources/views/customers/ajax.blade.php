            ...
            @foreach($customers as $key => $customer)
                <tr class="{{$customer->expired_time_boolean == 1 ? 'qua-han' : ''}}">
                    ...
                    <td class="text-center phone-customer {{in_array(3,$user_filter_grid) ? '':'display-none'}}" data-customer-id="{{ $customer->id }}">
                        <a href="javascript:void(0)" class="color-primary" id="callButton" data-customer-id="{{ $customer->id }}" data-phone="{{$customer->phone}}">{{\Illuminate\Support\Facades\Auth::user()->permission('phone.open') ? $customer->phone : str_limit($customer->phone, 7, 'xxx')}}
                        </a>
