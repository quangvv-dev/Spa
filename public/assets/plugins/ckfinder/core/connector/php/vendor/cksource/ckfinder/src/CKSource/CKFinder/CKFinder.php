...
class CKFinder extends Container implements HttpKernelInterface
{
    ...
    protected function registerPlugins()
    {
        ...
        foreach ($pluginsEntries as $pluginInfo) {
            ...
            if (!class_exists($pluginClassName)) {
                throw new InvalidPluginException(sprintf('CKFinder plugin "%s" not found (%s)', $pluginName,
                    $pluginClassName), array('pluginName' => $pluginName));
            }
            ...
            if ($pluginObject instanceof PluginInterface) {
                ...
                throw new InvalidPluginException(sprintf('CKFinder plugin class must implement %sPluginInterface',
                    CKFinder::PLUGINS_NAMESPACE), array('pluginName' => $pluginName));
            }
        }
    }
    ...
    protected function checkRequirements()
    {
        ...
        if (version_compare(PHP_VERSION, '5.6.0') < 0) {
            throw new CKFinderException(sprintf($errorMessage,
                'Your PHP version is too old. CKFinder 3.x requires PHP 5.6+.'), Error::CUSTOM_ERROR);
        }
        ...
        if (!empty($missingExtensions)) {
            throw new CKFinderException(sprintf($errorMessage,
                'Missing PHP extensions: ' . implode(', ', $missingExtensions) . '.'), Error::CUSTOM_ERROR);
        }
    }
