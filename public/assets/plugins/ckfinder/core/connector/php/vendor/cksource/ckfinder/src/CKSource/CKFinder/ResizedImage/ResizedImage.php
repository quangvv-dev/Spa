...
class ResizedImage extends ResizedImageAbstract
{
    ...
    public function __construct(
        ...
        parent::__construct($sourceFileResourceType, $sourceFileDir, $sourceFileName, $requestedWidth,
            $requestedHeight);
        ...
        if (!$forceRequestedSize) {
            ...
            if (null === $cachedInfo || !isset($cachedInfo['width']) || !isset($cachedInfo['height'])) {
                $sourceFilePath = Path::combine($sourceFileResourceType->getDirectory(), $sourceFileDir,
                    $sourceFileName);
            ...
            $targetSize = Image::calculateAspectRatio($requestedWidth, $requestedHeight, $originalImageWidth,
                $originalImageHeight);
    ...
    public static function createFilename($fileName, $width, $height)
    {
        ...
        return sprintf("%s__%dx%d%s", $pathInfo['filename'], $width, $height,
    ...
    {
        if (null === $this->image) {
            $sourceFilePath = Path::combine($this->sourceFileResourceType->getDirectory(), $this->sourceFileDir,
                $this->sourceFileName);
    ...
    public function getUrl()
    {
        ...
        return $backend->getFileUrl($this->sourceFileResourceType, $this->sourceFileDir, $this->sourceFileName,
            $this->getFileName());
    }
