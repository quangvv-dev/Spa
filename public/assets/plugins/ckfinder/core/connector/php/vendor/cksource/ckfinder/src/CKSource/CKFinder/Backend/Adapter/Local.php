...
class Local extends \League\Flysystem\Adapter\Local
{
    ...
    public function __construct(array $backendConfig)
    {
        ...
        if (!is_dir($backendConfig['root'])) {
            ...
            if (!is_dir($backendConfig['root'])) {
                throw new FolderNotFoundException(sprintf('The root folder of backend "%s" not found (%s)',
                    $backendConfig['name'], $backendConfig['root']));
            }
        }

        if (!is_readable($backendConfig['root'])) {
            throw new AccessDeniedException(sprintf('The root folder of backend "%s" is not readable (%s)',
                $backendConfig['name'], $backendConfig['root']));
        }
    ...
    public function convertToFilesystemEncoding($fileName)
    {
        ...
        if (!function_exists("iconv")) {
            if (strcasecmp($encoding, "ISO-8859-1") == 0 || strcasecmp($encoding,
                    "ISO8859-1") == 0 || strcasecmp($encoding, "Latin1") == 0) {
