...
abstract class PdoProfilerStorage implements ProfilerStorageInterface
{
    ...
    public function find($ip, $url, $limit, $method, $start = null, $end = null)
    {
        ...
        $tokens = $this->fetch($db,
            'SELECT token, ip, method, url, time, parent, status_code FROM sf_profiler_data ' . $criteria . ' ORDER BY time DESC LIMIT ' . ((int)$limit),
            $args);
        $this->close($db);
    ...
    public function write(Profile $profile)
    {
        ...
        try {
            if ($this->has($profile->getToken())) {
                ...
                $this->exec($db,
                    'INSERT INTO sf_profiler_data (token, parent, data, ip, method, url, time, created_at, status_code) VALUES (:token, :parent, :data, :ip, :method, :url, :time, :created_at, :status_code)',
                    $args);
            }
            $this->cleanup();
            $status = true;
