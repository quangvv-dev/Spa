...
class BackendFactory
{
    ...
    protected function registerDefaultAdapters()
    {
        ...
        $this->registerAdapter('s3', function ($backendConfig) {
            ...
            return $this->createBackend($backendConfig, new AwsS3Adapter($client, $backendConfig['bucket'], $prefix),
                $filesystemConfig);
        });
        ...
        $this->registerAdapter('azure', function ($backendConfig) {
            $endpoint = sprintf('DefaultEndpointsProtocol=https;AccountName=%s;AccountKey=%s',
                $backendConfig['account'], $backendConfig['key']);
            $blobRestProxy = ServicesBuilder::getInstance()->createBlobService($endpoint);

            $prefix = isset($backendConfig['root']) ? trim($backendConfig['root'], '/ ') : null;

            return $this->createBackend($backendConfig,
                new AzureAdapter($blobRestProxy, $backendConfig['container'], $prefix));
        });
    }
    ...
    public function getBackend($backendName)
    {
        ...
        if (!isset($this->registeredAdapters[$adapterName])) {
            throw new \InvalidArgumentException(sprintf('Backends adapter "%s" not found. Please check configuration file.',
                $adapterName));
        }

        if (!is_callable($this->registeredAdapters[$adapterName])) {
            throw new \InvalidArgumentException(sprintf('Backend instantiation callback for adapter "%s" is not a callable.',
                $adapterName));
        }
        ...
        if (!$backend instanceof Backend) {
            throw new CKFinderException(sprintf('The instantiation callback for adapter "%s" didn\'t return a valid Backend object.',
                $adapterName));
        }
    ...
    public function getPrivateDirBackend($privateDirIdentifier)
    {
        ...
        if (!array_key_exists($privateDirIdentifier, $privateDirConfig)) {
            throw new \InvalidArgumentException(sprintf('Private dir with identifier %s not found. Please check configuration file.',
