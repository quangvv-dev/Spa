...
class EditedFile extends ExistingFile
{
    ...
    public function __construct($fileName, CKFinder $app, $newFileName = null)
    {
        ...
        parent::__construct($fileName, $this->workingFolder->getClientCurrentFolder(),
            $this->workingFolder->getResourceType(), $app);
    }
    ...
    public function isValid()
    {
        ...
        if ($this->newContents) {
            if (Utils::containsHtml(substr($this->newContents, 0, 1024)) &&
                !in_array(strtolower($this->newFileName ? $this->getNewExtension() : $this->getExtension()),
                    $this->config->get('htmlExtensions'))) {
                throw new InvalidUploadException('HTML detected in disallowed file type',
