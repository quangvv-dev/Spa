...
class Config
{
    ...
    protected function validate()
    {
        ...
        $checkMissingNodes(array('backend', 'tags', 'logs', 'cache', 'thumbs'), $this->options['privateDir'],
            '[privateDir]');
    ...
    public function getHideFoldersRegex()
    {
        ...
        if (!isset($hideFoldersRegex)) {
            ...
            if ($hideFoldersConfig && is_array($hideFoldersConfig)) {
                ...
                $hideFoldersRegex = strtr($hideFoldersRegex,
                    array("?" => "__QMK__", "*" => "__AST__", "|" => "__PIP__"));
    ...
    public function getBackendNode($backendName)
    {
        if (array_key_exists($backendName, $this->options['backends'])) {
            ...
            throw new \InvalidArgumentException(sprintf('Backend %s not found. Please check configuration file.',
                $backendName));
        }
    }
}
