    ...
    public function getController(Request $request)
    {
        ...
        if (class_exists($commandClassName)) {
            ...
            if (!$reflectedClass->isInstantiable()) {
                throw new InvalidCommandException(sprintf('CKFinder command class %s is not instantiable',
                    $commandClassName));
            }
        ...
        if (!$commandObject instanceof CommandAbstract) {
            throw new InvalidCommandException(sprintf("CKFinder command must be a subclass of CommandAbstract (%s given)",
                get_class($commandObject)));
        }

        if (!method_exists($commandObject, self::COMMAND_EXECUTE_METHOD)) {
            throw new InvalidCommandException(sprintf("CKFinder command class %s doesn't contain required 'execute' method",
                $commandClassName));
        }
