...
class WorkingFolder extends Folder implements EventSubscriberInterface
{
    ...
    public function getAclMask()
    {
        if (null === $this->aclMask) {
            $this->aclMask = $this->app->getAcl()->getComputedMask($this->getResourceTypeName(),
    ...
    public function createDir($dirname)
    {
        ...
        if (!Folder::isValidName($dirname,
                $config->get('disallowUnsafeCharacters')) || $backend->isHiddenFolder($dirname)) {
    ...
    public function delete()
    {
        ...
        $this->app['cache']->deleteByPrefix(Path::combine($this->resourceType->getName(),
            $this->getClientCurrentFolder()));
    ...
    public function getFileUrl($fileName, $thumbnailFileName = null)
    {
        ...
        return $this->backend->getFileUrl($this->resourceType, $this->getClientCurrentFolder(), $fileName,
            $thumbnailFileName);
    }
    ...
    public function addCurrentFolderInfo(FilterResponseEvent $event)
    {
        ...
        if ($response instanceof JsonResponse) {
            ...
            if (null !== $baseUrl) {
                $folderUrl = Path::combine($baseUrl,
                    Utils::encodeURLParts(Path::combine($this->resourceType->getDirectory(),
                        $this->getClientCurrentFolder())));
