...
class ResizedImageRepository
{
    ...
    public function getResizedImage(
        ...
        if (!$this->acl->isAllowed($sourceFileResourceType->getName(), $sourceFileDir,
    ...
    {
        $resizedImagesPath = Path::combine($sourceFileResourceType->getDirectory(), $sourceFilePath, ResizedImage::DIR,
            $sourceFileName);
    ...
    public function copyResizedImages(
        ...
        $sourceResizedImagesPath = Path::combine($sourceFileResourceType->getDirectory(), $sourceFilePath,
            ResizedImage::DIR, $sourceFileName);
        $targetResizedImagesPath = Path::combine($targetFileResourceType->getDirectory(), $targetFilePath,
            ResizedImage::DIR, $targetFileName);
        ...
        if ($sourceBackend->hasDirectory($sourceResizedImagesPath)) {
            ...
            foreach ($resizedImages as $resizedImage) {
                ...
                $targetImageFilename = ResizedImage::createFilename($targetFileName, $sourceImageSize['width'],
                    $sourceImageSize['height']);

                $targetBackend->putStream(Path::combine($targetResizedImagesPath, $targetImageFilename),
                    $resizedImageStream);
            }
        }
    }
        ...
        if ($backend->hasDirectory($resizedImagesPath)) {
            if ($backend->rename($resizedImagesPath, $newResizedImagesPath)) {
                ...
                foreach ($resizedImages as $resizedImage) {
                    ...
                    $sourceImageSize = ResizedImage::getSizeFromFilename($resizedImage['basename']);
                    $newResizedImageFilename = ResizedImage::createFilename($newSourceFileName,
                        $sourceImageSize['width'], $sourceImageSize['height']);

                    $backend->rename($resizedImage['path'],
